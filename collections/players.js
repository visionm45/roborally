var player = {
    ...
    addDamage: function(inc) {
        if (this.hasOptionCard('ablative_coat')) {
            if (!this.ablativeCoat)
                this.ablativeCoat = 0;
            this.ablativeCoat++;
            console.log("ablative coat has been hit " + this.ablativeCoat + " times.");
            if (this.ablativeCoat == 3)  {
                console.log("ablative coat has been destroyed; removing from player.");
    ...
    drawOptionCard: function() {
        ...
        if (optionCards.length == 0)
        {
            ...
            this.chat("tried to grab an option card when none are left!");
            return;
        }
        var optionId = optionCards.pop();
        console.log("Drew option card " + optionId + ": " + CardLogic.getOptionName(optionId) + "; " + optionCards.length + " left in deck");
    ...
    discardOptionCard: function(name) {
        var gameId = this.game()._id;
        console.log("discarding " + name);
        console.log("this.optionCards[" + name + "] before: " + this.optionCards[name]);
        delete this.optionCards[name];
        //console.log("this.optionCards[" + name + "] after delete:  " + this.optionCards[name]);
